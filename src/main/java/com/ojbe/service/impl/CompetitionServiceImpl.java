...
import com.ojbe.constant.CommonConstant;
import com.ojbe.exception.BusinessException;
import com.ojbe.mapper.*;
...
import com.ojbe.model.vo.*;
import com.ojbe.mq.JudgeProducer;
import com.ojbe.service.interfaces.CompetitionRankingService;
import com.ojbe.service.interfaces.CompetitionService;
import com.ojbe.service.interfaces.ReputationService;
...
import com.ojbe.service.interfaces.UserService;
import com.ojbe.utils.TimeFormatUtil;
...
public class CompetitionServiceImpl extends ServiceImpl<CompetitionMapper, Competition>
    ...
    public Long createContest(ContestCreateRequest request, Long userId) {
        ...
        if (creator != null && !"admin".equals(creator.getRole())) {
            ...
            if (level < com.ojbe.constant.LevelConstants.CREATE_PRIVATE_LEVEL) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR,
                        "声望不足，Lv1 初出茅庐以上可创建比赛（当前声望: " +
                        (creator.getReputation() != null ? creator.getReputation() : 0) + "）");
            }
            if (level < com.ojbe.constant.LevelConstants.CREATE_PUBLIC_LEVEL
        ...
        if (StringUtils.isNotBlank(request.getPassword())) {
            contest.setPassword(com.ojbe.utils.PasswordUtil.encryptPassword(request.getPassword()));
        }
    ...
    public Boolean register(Long contestId, Long userId, String password) {
        ...
        if (StringUtils.isNotBlank(contest.getPassword())) {
            if (StringUtils.isBlank(password) ||
                    !com.ojbe.utils.PasswordUtil.verifyPassword(password, contest.getPassword())) {
